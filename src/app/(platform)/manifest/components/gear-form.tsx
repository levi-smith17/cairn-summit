'use client'

import { useState, useRef, useEffect } from 'react'
import { useForm } from 'react-hook-form'
import { zodResolver } from '@hookform/resolvers/zod'
import { z } from 'zod'
import { saveGear, deleteGear } from '@/actions/manifest'
import {
  Form,
  FormControl,
  FormField,
  FormItem,
  FormLabel,
  FormMessage,
} from '@/components/ui/form'
import { Input } from '@/components/ui/input'
import { Button } from '@/components/ui/button'
import { EntryActions } from './entry-actions'
import { FormActions } from '@/components/forms/form-actions'
import { CustomSelect } from '@/components/ui/custom-select'
import { Separator } from '@/components/ui/separator'
import { Popover, PopoverContent, PopoverTrigger } from '@/components/ui/popover'
import { Check, ChevronDown, Plus } from 'lucide-react'

const gearLevels = ['BEGINNER', 'INTERMEDIATE', 'ADVANCED', 'EXPERT'] as const
type GearLevel = typeof gearLevels[number]

const gearSchema = z.object({
  id: z.string().optional(),
  name: z.string().min(1, 'Name is required'),
  category: z.string().optional(),
  level: z.enum(gearLevels).optional(),
})

type GearFormValues = z.infer<typeof gearSchema>

interface Gear {
  id: string
  name: string
  category: string | null
  level: GearLevel | null
}

interface GearFormProps {
  gear: Gear[]
  adding: boolean
  setAdding: (v: boolean) => void
  saving: boolean
  saved: boolean
  error: boolean
  handleSubmit: (action: () => Promise<void>) => Promise<void>
}

const emptyDefaults: GearFormValues = {
  name: '',
  category: '',
  level: undefined,
}

const levelLabels: Record<GearLevel, string> = {
  BEGINNER: 'Beginner',
  INTERMEDIATE: 'Intermediate',
  ADVANCED: 'Advanced',
  EXPERT: 'Expert',
}

function CreatableCategorySelect({
  value,
  onChange,
  options,
}: {
  value: string
  onChange: (val: string) => void
  options: string[]
}) {
  const [open, setOpen] = useState(false)
  const [inputValue, setInputValue] = useState('')

  const sorted = [...options].sort((a, b) => a.localeCompare(b))
  const filtered = inputValue
    ? sorted.filter(o => o.toLowerCase().includes(inputValue.toLowerCase()))
    : sorted
  const showCreate =
    inputValue.trim() !== '' &&
    !sorted.some(o => o.toLowerCase() === inputValue.toLowerCase().trim())

  function handleSelect(val: string) {
    onChange(val)
    setInputValue('')
    setOpen(false)
  }

  function handleCreate() {
    const newVal = inputValue.trim()
    if (newVal) {
      onChange(newVal)
      setInputValue('')
      setOpen(false)
    }
  }

  return (
    <Popover
      open={open}
      onOpenChange={o => {
        setOpen(o)
        if (!o) setInputValue('')
      }}
    >
      <PopoverTrigger asChild>
        <Button
          type="button"
          variant="outline"
          size="sm"
          className="h-9 gap-1.5 text-sm justify-start w-full"
        >
          <span className={`flex-1 text-left truncate ${!value ? 'text-muted-foreground' : ''}`}>
            {value || 'Select or add category'}
          </span>
          <ChevronDown className="h-3 w-3 shrink-0 text-muted-foreground" />
        </Button>
      </PopoverTrigger>
      <PopoverContent className="w-56 p-0 overflow-hidden" align="start" sideOffset={4}>
        <div className="p-2 border-b">
          <Input
            autoFocus
            placeholder="Search or create…"
            value={inputValue}
            onChange={e => setInputValue(e.target.value)}
            onKeyDown={e => {
              if (e.key === 'Enter') {
                e.preventDefault()
                if (showCreate) handleCreate()
                else if (filtered.length === 1) handleSelect(filtered[0])
              }
            }}
            className="h-7 text-xs"
          />
        </div>
        <div className="max-h-48 overflow-y-auto py-1">
          {showCreate && (
            <button
              type="button"
              onClick={handleCreate}
              className="flex items-center gap-2 w-full px-3 py-1.5 text-xs hover:bg-muted/40 text-left"
            >
              <Plus className="h-3 w-3 shrink-0" />
              <span>Create &ldquo;{inputValue.trim()}&rdquo;</span>
            </button>
          )}
          {filtered.map(opt => (
            <button
              type="button"
              key={opt}
              onClick={() => handleSelect(opt)}
              className={`flex items-center gap-2.5 w-full px-3 py-1.5 text-xs transition-colors text-left ${
                opt === value ? 'bg-muted/50' : 'hover:bg-muted/40'
              }`}
            >
              <span className="flex-1 truncate">{opt}</span>
              {opt === value && <Check className="h-3 w-3 shrink-0 text-foreground/60" />}
            </button>
          ))}
          {filtered.length === 0 && !showCreate && (
            <p className="px-3 py-2 text-xs text-muted-foreground">No categories yet</p>
          )}
        </div>
      </PopoverContent>
    </Popover>
  )
}

export function GearForm({ gear, adding, setAdding, saving, saved, error, handleSubmit }: GearFormProps) {
  const [editing, setEditing] = useState<string | null>(null)
  const containerRef = useRef<HTMLDivElement>(null)

  const form = useForm<GearFormValues>({
    resolver: zodResolver(gearSchema),
    defaultValues: emptyDefaults,
  })

  useEffect(() => {
    if (adding) {
      containerRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })
    }
  }, [adding])

  function startEdit(entry: Gear) {
    form.reset({
      id: entry.id,
      name: entry.name,
      category: entry.category ?? '',
      level: entry.level ?? undefined,
    })
    setAdding(false)
    setEditing(entry.id)
    containerRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })
  }

  function cancel() {
    form.reset(emptyDefaults)
    setEditing(null)
    setAdding(false)
  }

  async function onSubmit(values: GearFormValues) {
    await handleSubmit(async () => {
      await saveGear({
        id: values.id,
        name: values.name,
        category: values.category ?? null,
        level: values.level ?? null,
      })
      cancel()
    })
  }

  async function onDelete(id: string) {
    await deleteGear(id)
  }

  const existingCategories = [...new Set(gear.map(g => g.category).filter(Boolean) as string[])]

  const grouped = gear.reduce<Record<string, Gear[]>>((acc, entry) => {
    const key = entry.category ?? 'Uncategorized'
    if (!acc[key]) acc[key] = []
    acc[key].push(entry)
    return acc
  }, {})

  const sortedGroupEntries = Object.entries(grouped).sort(([a], [b]) => a.localeCompare(b))

  return (
    <div ref={containerRef} className="space-y-6">
      {/* Add/Edit form */}
      {(adding || editing) && (
        <>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="rounded-lg border bg-secondary p-4 space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <FormField
                  control={form.control}
                  name="name"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Skill</FormLabel>
                      <FormControl>
                        <Input placeholder="e.g. TypeScript" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                />
                <FormField
                  control={form.control}
                  name="category"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Category</FormLabel>
                      <CreatableCategorySelect
                        value={field.value ?? ''}
                        onChange={field.onChange}
                        options={existingCategories}
                      />
                      <FormMessage />
                    </FormItem>
                  )}
                />
                <FormField
                  control={form.control}
                  name="level"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>Level</FormLabel>
                      <CustomSelect
                        options={gearLevels.map(l => ({ value: l, label: levelLabels[l] }))}
                        value={field.value ?? ''}
                        onChange={field.onChange}
                        placeholder="Select level"
                        triggerClassName="w-full"
                      />
                      <FormMessage />
                    </FormItem>
                  )}
                />
              </div>

              <div className="flex justify-end items-center gap-4">
                <Button type="button" variant="ghost" onClick={cancel}>
                  Cancel
                </Button>
                <FormActions
                  saving={saving}
                  saved={saved}
                  error={error}
                  saveLabel={editing ? 'Save Changes' : 'Add Gear'}
                  hideAlert
                />
              </div>
            </form>
          </Form>
          <Separator />
        </>
      )}

      {/* Existing entries grouped by category (sorted alphabetically) */}
      {gear.length > 0 && (
        <div className="space-y-6">
          {sortedGroupEntries.map(([category, items]) => (
            <div key={category}>
              <p className="text-sm font-medium text-muted-foreground mb-2">{category}</p>
              <div className="flex flex-wrap gap-2">
                {items.map((entry) => (
                  <div
                    key={entry.id}
                    className="flex items-center gap-2 rounded-full border px-3 py-1 text-sm"
                  >
                    <span>{entry.name}</span>
                    {entry.level && (
                      <span className="text-muted-foreground text-xs">
                        · {levelLabels[entry.level]}
                      </span>
                    )}
                    <EntryActions
                      onEdit={() => startEdit(entry)}
                      onDelete={() => onDelete(entry.id)}
                      deleteTitle="Remove Gear"
                      itemName={entry.name}
                    />
                  </div>
                ))}
              </div>
            </div>
          ))}
        </div>
      )}
    </div>
  )
}
