...
import { sendInvitation, revokeInvitation } from '@/actions/admin'
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
  ...
  return (
    <div className="flex items-start gap-3 py-3 px-4">
      ...
      <div className="flex items-center gap-0.5 shrink-0">
        {!used && !expired && <CopyLink token={token} />}
        {!used && (
          <Button
            variant="ghost"
            size="icon"
...
export function InvitationsPanel({
  ...
  invitations: InvitationSummary[]
  tokens: Record<string, string> // id → token (only for unused/pending)
}) {
          ...
          <>
            {pending.length > 0 && (
              <>
                ...
                {pending.map((inv, i) => (
                  <div key={inv.id}>
                    <InvitationRow inv={inv} token={tokens[inv.id] ?? ''} onRevoke={setRevokeId} />
