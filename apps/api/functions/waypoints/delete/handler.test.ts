...
describe('waypoints/delete handler', () => {
    beforeEach(() => {
        vi.clearAllMocks()
    })
    ...
    it('returns 204 on successful delete', async () => {
        const result = await handler(mockEvent('user-123', 'wp-abc')) as any
        expect(result.statusCode).toBe(204)
    })

    it('deletes the waypoint with the correct key', async () => {
        await handler(mockEvent('user-123', 'wp-abc'))
