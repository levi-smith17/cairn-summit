...
exports.fetchCalDavEvents = fetchCalDavEvents;
const ical_1 = require("./ical");
const CALDAV_NS = 'urn:ietf:params:xml:ns:caldav';
function basicAuth(username, password) {
    return `Basic ${Buffer.from(`${username}:${password}`).toString('base64')}`;
}
async function davRequest(method, url, auth, body, depth) {
    const headers = {
        Authorization: auth,
...
async function getPrincipalUrl(serverUrl, auth) {
...
async function getCalendarHome(principalUrl, auth) {
    const body = `<?xml version="1.0" encoding="UTF-8"?>
<d:propfind xmlns:d="DAV:" xmlns:cal="${CALDAV_NS}">
  <d:prop><cal:calendar-home-set/></d:prop>
    ...
    if (!href)
        throw new Error('CalDAV calendar-home-set not found');
...
async function fetchCalendarObjects(calendarUrl, auth, from, to) {
    const body = `<?xml version="1.0" encoding="UTF-8"?>
...
async function fetchCalDavEvents(serverUrl, appleId, password, calendarName, from, to) {
    ...
    const target = calendars.find(c => c.displayName.toLowerCase() === calendarName.toLowerCase());
    if (!target)
        return [];
    const icsBlocks = await fetchCalendarObjects(target.url, auth, from, to);
    return icsBlocks.flatMap(ics => (0, ical_1.parseICSEvents)(ics, target.url, from, to));
}
