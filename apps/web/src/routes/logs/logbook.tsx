...
export function Logbook({
  ...
  const titleAutoSaveTimer = useRef<ReturnType<typeof setTimeout> | null>(null)
  ...
  function goToLog(targetIndex: number, dir: 'next' | 'prev') {
    if (isDirty) {
  ...
  async function doAddPage(saveFirst: boolean) {
    ...
    const newIndex = localLogs.length
    try {
      ...
      {!reorderMode && (
        <div className="flex items-center justify-between px-4 py-2 border-b shrink-0 bg-muted/20">
          <Button
            ...
            onClick={() => goToLog(currentIndex - 1, 'prev')} disabled={!canPrev}
          >
                          ...
                          <SortablePageRow
                            ...
                            onNavigate={() => {
                              ...
                              if (realIdx !== currentIndex) {
                                goToLog(realIdx, realIdx > currentIndex ? 'next' : 'prev')
                              }
                            }}
                          />
                        )
          ...
          <Button
            variant="ghost" size="sm" className="h-7 gap-1 text-xs"
            onClick={() => goToLog(currentIndex + 1, 'next')} disabled={!canNext}
          >
            Next
