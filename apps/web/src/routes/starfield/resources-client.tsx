...
export function ResourcesClient({ resources, resourceTypes, totalCount, currentPage, pageSize, onRefresh }: ResourcesClientProps) {
  ...
  function handleAdd() { setEditingResource(null); setMode('add') }
  function handleEdit(resource: any) { setEditingResource(resource); setMode('edit') }
  function handleFormDone() { setMode('view'); setEditingResource(null) }
      ...
      <div className="flex flex-col flex-1 gap-4 p-4 overflow-hidden min-h-0">
        ...
        <div className="flex flex-1 gap-4 overflow-hidden min-h-0">
          <div className={`${(selectedResource || showForm) ? 'hidden md:flex' : 'flex'} flex-col w-full md:w-1/3 rounded-lg border border-border bg-card overflow-hidden`}>
            <ResourceList resources={filteredResources} resourceTypes={resourceTypes} selectedResourceId={selectedResourceId} onSelect={selectResource} onNew={handleAdd} onEdit={handleEdit} onDelete={(id, name) => setDeleteTarget({ id, name })} totalCount={totalCount} currentPage={currentPage} pageSize={pageSize} isSearching={!!search.trim()} />
          </div>
          <div className={`${(selectedResource || showForm) ? 'flex' : 'hidden md:flex'} flex-col flex-1 rounded-lg border border-border bg-card overflow-hidden`}>
            {showForm ? (
              <ResourceForm key={editingResource?.id ?? 'new'} resource={editingResource} resources={resources} resourceTypes={resourceTypes} onDone={handleFormDone} onRefresh={onRefresh} />
            ) : (
              <ResourceInfo resource={selectedResource} onBack={clearResource} onEdit={handleEdit} onDelete={(id, name) => setDeleteTarget({ id, name })} />
            )}
          </div>
        </div>
      </div>
