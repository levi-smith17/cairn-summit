...
interface ManifestContactInfoProps {
  wayfarer: { username: string; name: string | null; email: string | null; avatar: string | null }
  origins: { headline: string | null; summary: string | null; location: string | null; website: string | null; linkedin: string | null; github: string | null } | null
}

export function ManifestContactInfo({ wayfarer, origins }: ManifestContactInfoProps) {
  return (
    <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
      ...
      <div className="flex items-center gap-1">
        ...
        <Link to={`/manifest/${wayfarer.username}/contact`} className="hover:text-foreground underline underline-offset-4">
          Contact {wayfarer.name ?? wayfarer.username}
        </Link>
      </div>
      {origins?.website && (
        <div className="flex items-center gap-1">
