...
const formatDate = (date: string | Date) => format(toLocalDate(date), 'MMM yyyy')
...
interface ManifestContentProps {
  ...
  expeditions: { id: string; title: string; company: string; location: string | null; startDate: string; endDate: string | null; current: boolean; description: string | null }[]
  training: { id: string; institution: string; degree: string | null; field: string | null; startDate: string; endDate: string | null; current: boolean; description: string | null }[]
  gear: { id: string; name: string; category: string | null; level: string | null }[]
...
function DateRange({ startDate, endDate, current }: { startDate: string; endDate: string | null; current: boolean }) {
  return (
    ...
    <div className="rounded-lg border p-4 flex flex-col gap-2 bg-secondary">
      <div className="flex items-start justify-between gap-2">
        ...
        <div className="flex items-center gap-2 shrink-0">
          {l.url && (
            <a href={l.url} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors">
              <ExternalLink className="h-3 w-3" />
            </a>
          )}
          {l.githubUrl && (
            <a href={l.githubUrl} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors">
              <GitBranch className="h-3 w-3" />
            </a>
          )}
        </div>
      </div>
      <div ref={innerRef} className={`relative ${!expanded ? 'max-h-48 overflow-hidden' : ''}`}>
        {l.startDate && <DateRange startDate={l.startDate} endDate={l.endDate} current={l.current} />}
        {l.description && <RichTextContent html={l.description} className="text-muted-foreground text-sm" />}
        {!expanded && overflows && (
          <div className="absolute bottom-0 left-0 right-0 h-10 bg-gradient-to-t from-secondary to-transparent pointer-events-none" />
        )}
      </div>
      {overflows && (
        <button onClick={() => setExpanded(e => !e)} className="text-xs text-muted-foreground hover:text-foreground self-start transition-colors">
      ...
      <div className="max-w-3xl mx-auto px-6 pb-6 flex flex-col gap-12 print:max-w-none print:px-0 print:mx-0 print:pb-0">
        ...
        {expeditions.length > 0 && (
          <Section title={terms.expeditions}>
            <Timeline items={expeditions.map(exp => ({ title: exp.title, subtitle: exp.company, location: exp.location, startDate: new Date(exp.startDate), endDate: exp.endDate ? new Date(exp.endDate) : null, current: exp.current, description: exp.description, formatDate }))} />
          </Section>
        )}

        {training.length > 0 && (
          <Section title={terms.training}>
            <Timeline items={training.map(t => ({ title: t.institution, subtitle: t.degree ?? undefined, location: t.field ?? undefined, startDate: new Date(t.startDate), endDate: t.endDate ? new Date(t.endDate) : null, current: t.current, description: t.description, formatDate }))} />
          </Section>
        )}
              ...
              {Object.entries(grouped).map(([category, items]) => (
                <div key={category} className="flex flex-col gap-1">
                  ...
                  <ul className="text-sm text-muted-foreground list-none p-0 m-0 flex flex-col gap-0.5">
                    {items.map(item => <li key={item.id}>{item.name}{item.level ? ` — ${item.level}` : ''}</li>)}
                  </ul>
                </div>
              ))}
            </div>
        ...
        {landmarks.length > 0 && (
          <Section title={terms.landmarks}>
            <div className="flex flex-col gap-4 sm:hidden">
              {landmarks.map(l => <LandmarkCard key={l.id} l={l} />)}
            </div>
            <div className="hidden sm:flex gap-4">
              <div className="flex-1 flex flex-col gap-4">
                {landmarks.filter((_, i) => i % 2 === 0).map(l => <LandmarkCard key={l.id} l={l} />)}
              </div>
              <div className="flex-1 flex flex-col gap-4">
                {landmarks.filter((_, i) => i % 2 === 1).map(l => <LandmarkCard key={l.id} l={l} />)}
              </div>
            </div>
          </Section>
        )}
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="font-medium">
                      {s.title}
                      {s.url && (
                        <a href={s.url} target="_blank" rel="noopener noreferrer" className="ml-1.5 inline-block align-middle text-muted-foreground hover:text-foreground transition-colors">
                          <ExternalLink className="h-3 w-3" />
                        </a>
                      )}
                    </p>
                  ...
                  {s.date && <span className="text-sm text-muted-foreground shrink-0">{formatDate(s.date)}</span>}
                </div>
                {s.description && <RichTextContent html={s.description} className="text-muted-foreground" />}
              </div>
            ))}
          </Section>
        )}
          ...
          <Section title={terms.pathfinding}>
            {pathfinding.map(p => (
              <div key={p.id} className="flex flex-col gap-1">
                ...
                {p.description && <RichTextContent html={p.description} className="text-muted-foreground" />}
              </div>
            ))}
          </Section>
        )}
