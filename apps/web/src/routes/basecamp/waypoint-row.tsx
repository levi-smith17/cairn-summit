...
import { deleteWaypoint, toggleWaypointRead, toggleWaypointReadLater } from '@/lib/api/waypoints'
...
export function WaypointRow({ waypoint, folders, tags, onRefresh }: WaypointRowProps) {
        ...
        <div className="ml-7 mr-4 mb-2">
          <div className="border rounded-lg overflow-hidden">
            <InlineLogForm
              ...
              onSaved={() => { setAddingLog(false); onRefresh?.() }}
            />
          </div>
        </div>
      )}
      ...
      {waypoint.logs?.length > 0 && (
        <div className="flex flex-col divide-y border rounded-lg ml-7 mr-4 mb-2 overflow-hidden">
          {waypoint.logs.map((log: any) => (
            <LogRow key={log.id} log={log} folders={folders} tags={tags} onRefresh={onRefresh} />
          ))}
        </div>
      )}
